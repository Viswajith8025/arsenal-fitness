...
const SectionHeader = ({ subtitle, title, titleAccent, centered = true, className = "" }) => {
  return (
    <div className={`space-y-4 ${centered ? 'text-center' : ''} ${className}`}>
      {subtitle && (
        <span className="text-blue-500 text-[10px] font-black uppercase tracking-[0.6em] block mb-2 px-1">
          {subtitle}
        </span>
      )}
      <h2 className="text-4xl md:text-7xl font-black uppercase tracking-tight text-white leading-[1.1] md:leading-[0.95]">
        {title}{' '}
