      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12">
        ...
        <div className="flex flex-col items-center md:items-start gap-4">
          <div className="flex items-center gap-4 cursor-pointer group" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            <img src="/images/Logo.jpeg" alt="Arsenal Fitness Logo" className="w-8 h-8 object-contain transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-110" />
            <span className="text-lg font-black uppercase tracking-tighter text-white transition-all duration-300 group-hover:text-white">Arsenal <span className="text-blue-500">Fitness</span></span>
          </div>
          <p className="text-[10px] text-gray-600 font-bold uppercase tracking-[0.2em] transition-colors duration-300 hover:text-gray-500">Opal Complex, Malaparamba, Kozhikode</p>
          <a 
            href="tel:+917306675524" 
            className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-600 hover:text-blue-400 transition-all duration-300"
          >
        ...
        <div className="flex flex-wrap justify-center gap-8 md:gap-12">
          {footerLinks.map((link) => (
            <a 
              key={link.name} 
              href={link.href} 
              className="text-[10px] font-black uppercase tracking-widest text-gray-600 hover:text-white transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] relative after:absolute after:bottom-[-4px] after:left-0 after:h-px after:w-0 after:bg-blue-500 hover:after:w-full after:transition-all after:duration-500"
            >
              {link.name}
            </a>
          ))}
